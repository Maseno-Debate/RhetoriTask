...
function processSpeakers(speakers_sheet) {
  ...
  * Return: rooms (object with the selected rooms and their codenames)
  */
  ...
  function processRooms(roomsSheet, roomsNumber){
    ...
    for (var i = 1; i < roomsNumber; i++){
      var roomNumber = data[i][0];
      var codeName = data[i][1];
  
      rooms[roomNumber] = codeName;
    }
    return rooms;
  }
  ...
  */
  ...
  function randomPlacement(speakers, judges, rooms){
    var placement = {};
    var speakersPerRoom = Math.round(rooms.length / speakers.length);
    var remainingSpeakers = rooms.length % speakers.length;
    var judgesPerRoom = Math.round(rooms.length / judges.length);
    var remainingJudges = rooms.length % judges.length;
  
    Logger.log(remainingJudges)
    ...
    for (var i = 0; i < rooms.length; i++){
      ...
      if (i == rooms.length - 1){
        ...
        if (remainingJudges == 1){
          roomJudges.push(judges[0]);
        }
        else if (remainingJudges > 1){
          for (var m = 0; m < judges.length; m++){
            roomJudges.push(judges[m]);
          }
        }
      }
      placement[rooms[i]] = [roomJudges, roomSpeakers];
    }
    return placement;
  }
  ...
  */
  ...
  function createRound(placement, roundName){
    if (placement && roundName){
      SpreadsheetApp.getUi().alert(`Both placement and roundName Exist! Placement: ${placement} roundName: ${roundName}`);
    }
    ...
    for (var key in placement){
      var judgeArr = placement[key][0];
      var speakersArr = placement[key][1];
      ...
      roomCell.setValue(key);
  
      // Update room row value in prep for next key
      if (judgeNumbers > speakerNumbers){
        roomRow += judgeNumbers;
      }
      if (speakerNumbers > judgeNumbers){
        roomRow += speakerNumbers;
      }
  
      // Set speakers in second column and subsequent rows(cells)
      for (var i = 0; i < speakerNumbers; i++){
        var speakerCell = newSheet.getRange(x, (y + 1));
        speakerCell.setValue(speakersArr[i]);
        x++;
      }
      // Set judges in third column and subsequent rows(cells)
      for (var j = 0; j < judgeNumbers; j++){
        var judgeCell = newSheet.getRange(xx, (y + 2));
        judgeCell.setValue(judgeArr[j]);
        xx++;
      }
  ...
  function drawRound(){
    ...
    if (roundName && numberRooms && speakerSheetName && judgeSheetName && roomSheetName){
      ...
      if (speakersSheet && judgeSheet && roomSheet){
        var speakers = processSpeakers(speakersSheet); // TODO: Array of speakers needs to be object with experience levels
        var judgesObj = processJudges(judgeSheet); // Object of judges and experience levels
        var roomsObj = processRooms(roomSheet, numberRooms); //Object rooms and their codenames
        var judges = getObjKeys(judgesObj);
        var rooms = getObjKeys(roomsObj);
  
        var placement = randomPlacement(speakers, judges, rooms);
  ...
  function trial(){
    var speakers = ["Speaker1", "Speaker2", "Speaker3"];
    var judges = ["JudgeA", "JudgeB", "JudgeC"];
